# This repository is forked by NuclearGhost
...
- added deployment by instance-id (to support ssh over ssm)

### Installation:

- add `gem 'cap-ec2', git: 'https://github.com/nuclearghost/cap-ec2.git'` to your `Gemfile`
...
In the "staging" stage, all roles would apply _only_ to `server-4`.
...
You can pass options when defining your roles. The options are _exactly_ the same as the options that
...
you will be deploying to. Note that unlike Capistrano 2.x, all tasks _require_ a stage.
